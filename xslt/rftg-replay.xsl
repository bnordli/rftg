...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/RftgExport">
    <html>
      ...
      <body>
        ...
        <xsl:apply-templates select="Status/Message" mode="statusFooter" />
        <xsl:apply-templates select="Links[@text='Navigate']" />
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="Message" mode="statusFooter">
    <p><b><xsl:value-of select="../../PlayerName" />: <xsl:value-of select="." /></b></p>
  </xsl:template>
