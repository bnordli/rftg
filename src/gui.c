...
/* The original number of cards in variants with separate piles */
...
static void apply_options(void)
{
	...
	/* Find the minimum and maximum expansion */
	min_exp = min_expansion(real_game.variant);
	max_exp = max_expansion(real_game.variant);

	/* Sanity check expansions */
	if (real_game.expanded < min_exp) real_game.expanded = min_exp;
	if (real_game.expanded > max_exp) real_game.expanded = max_exp;
...
int main(int argc, char *argv[])
{
	...
	game_list = gtk_tree_store_new(GAME_MAX_COLUMN,
		...
		G_TYPE_STRING, //  4: Create name/Offline note collation key
		...
		G_TYPE_INT,    // 14: Variant id
		G_TYPE_STRING, // 15: Variant string
		G_TYPE_INT,    // 16: Game speed
		G_TYPE_INT,    // 17: My game/player id?
		G_TYPE_INT,    // 18: Checkboxes visible?
		G_TYPE_INT);   // 19: Weight of text
