 ...
 */
int settle_check_takeover(game *g, int who, int no_ask)
{
	player *p_ptr;
	card *c_ptr;
	power_where w_list[100];
...
void settle_chosen(game *g, int who, int first, int c_idx, int o_idx)
{
	...
	if (o_ptr->code & P3_PLACE_TWO)
	{
		...
		if (p_ptr->placing == -1)
		{
			...
			if (settle_check_takeover(g, who, 0))
			{
				/* Act on declaration */
...
static int settle_action(game *g, int who, int world)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (world != -1 && (o_ptr->code & P3_PLACE_TWO))
		{
			...
			if (settle_check_takeover(g, who, 1))
			{
				/* Add power to list */
