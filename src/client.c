...
void game_view_changed(GtkTreeView *view, gpointer data)
{
	...
	int owned, variant, minp, maxp, user = 1, self, nump;
	...
	gtk_tree_model_get(GTK_TREE_MODEL(game_list), &parent_iter,
	                   GAME_COL_VARIANT, &variant,
	                   GAME_COL_SELF, &owned,
	                   GAME_COL_MIN_PLAYERS, &minp,
	                   GAME_COL_MAX_PLAYERS, &maxp, -1);
...
static void handle_open_game(char *ptr, int size)
{
	...
	/* Set variant */
	gtk_tree_store_set(game_list, &list_iter, GAME_COL_VARIANT, x,
	                   GAME_COL_VARIANT_STR, variant_labels[x], -1);
...
static gboolean message_read(gpointer data)
{
	...
	switch (type)
	{
		...
		case MSG_PLAYER_NEW:
			...
			/* Check for self flag (since 0.8.1n) */
